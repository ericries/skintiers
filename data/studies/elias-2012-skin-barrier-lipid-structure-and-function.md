---
analyzed: '2026-08-09'
assurance: sonnet
name: Elias 2012 skin barrier lipid structure and function
slug: elias-2012-skin-barrier-lipid-structure-and-function
status: published
type: study
updated: '2026-08-09'
---

A short invited commentary by Peter Elias summarizing that the stratum corneum extracellular matrix is organized into lipid lamellar bilayers whose molecular architecture is now known, but that how the matrix's non-lipid components (enzymes, structural proteins, antimicrobial peptides) jointly govern barrier function remains largely unknown.[^1]

**Design:** Not a primary research study. PubMed classifies this as a "Comment"/invited perspective piece, not a study with original experiments, in the Journal of Investigative Dermatology.[^1]

**Population / n:** Not applicable; no subjects, animals, or samples were tested. The abstract describes no experiment.[^1]

**Intervention / comparator:** None; there is no intervention or comparator, as no experiment is reported.[^1]

**Primary result (verbatim figures):** "The stratum corneum extracellular matrix (ECM) is enriched in lipids that are organized into lamellar bilayers, whose molecular architecture is now known." The abstract adds that the bilayers matter for the permeability barrier, but the ECM also holds enzymes, structural proteins, and antimicrobial peptides that "impact barrier function," and that "how such diverse components affect barrier function remains largely unknown." "Static models of the epidermis may not do justice to the ECM, which is metabolically active, as it changes both structure and function as it transits to the surface." The abstract states no n, percentage, fold-change, or p-value; none is applicable to a commentary of this kind.[^1]

**Effect size / interpretation:** Not applicable; this piece reports no measured effect. It is conceptual: it frames the stratum corneum lipid bilayer architecture as established science while flagging the interaction of lipids with enzymes, structural proteins, and antimicrobial peptides in that matrix as an open question.[^1]

**Applicability + key limitation:** Useful as a compact conceptual anchor for the lamellar-bilayer model underlying [[ceramides]] and the [[skin-barrier-repair]] / [[skin-barrier-strengthening]] pages, but it is a brief invited commentary rather than a data-bearing study, so it supplies no sample size, intervention, or quantitative effect size to cite; only the PubMed/PMC abstract was accessed (published 2012, per its PMID/PMCID record), and the full text was not reviewed for this entry.

## Sources

[^1]: Elias PM. Structure and function of the stratum corneum extracellular matrix. J Invest Dermatol. 2012;132(9):2131-2133. DOI: 10.1038/jid.2012.246. PMID 22895445. PMCID PMC3587970. https://pubmed.ncbi.nlm.nih.gov/22895445/ (accessed 2026-08-08)